...
# The Python stack runs from its venv when present, else the ambient python3.
PY_BIN="$ROOT/packages/core-py/.venv/bin/python"
[[ -x "$PY_BIN" ]] || PY_BIN="$(command -v python3)"
py_run() {
  PYTHONPATH="$ROOT/packages/core-py/src" "$PY_BIN" "$ROOT/packages/core-py/tools/$1.py" "${@:2}"
}

# compare4 <label> <ts> <rs> <go> <py> [expected]
# Reports OK only when all stacks agree (and match `expected` when supplied).
compare4() {
  local label="$1" ts="$2" rs="$3" go="$4" py="$5" expected="${6-}"
  if [[ "$ts" == "$rs" && "$rs" == "$go" && "$go" == "$py" ]]; then
...
echo "Canonical bytes for shared TIM fixtures: TS vs Rust vs Go vs Python"
for fx in "${FIXTURES[@]}"; do
  ts="$(ts_run canon "$ROOT/$fx")"
  rs="$(rs_run canon "$ROOT/$fx")"
  go="$(go_run canon "$ROOT/$fx")"
  py="$(py_run canon "$ROOT/$fx")"
  if [[ "$ts" == "$rs" && "$rs" == "$go" && "$go" == "$py" ]]; then
    echo "[OK]   $fx (all stacks agree, $(printf '%s' "$py" | wc -c | tr -d ' ') bytes)"
  else
...
echo "All fixtures: TS, Rust, Go and Python canonical bytes are identical."

echo
echo "RFC 8785 number edges (exponent + precision): TS vs Rust vs Go vs Python"
...
for j in "${NUMS[@]}"; do
  compare4 "$j" "$(ts_run canonjson "$j")" "$(rs_run canonjson "$j")" \
    "$(go_run canonjson "$j")" "$(py_run canonjson "$j")"
done
check_fail "Cross-language number MISMATCH."
echo "All number edges: TS, Rust, Go and Python agree."

echo
echo "C2 canonicalization vectors: all four stacks vs published expectation"
for vec in "$ROOT"/vectors/canonicalization/c2-*.json; do
  ...
  compare4 "$name" "$(ts_run canonjson "$input")" "$(rs_run canonjson "$input")" \
    "$(go_run canonjson "$input")" "$(py_run canonjson "$input")" "$expected"
done
check_fail "Cross-language C2 vector MISMATCH."
echo "All C2 vectors: every stack matches the published expectation."

echo
echo "RFC 3339 timestamp edges (offset + fractional + rejection): all four stacks"
# The TS kernel uses Date.parse; Rust, Go and Python hand-roll a parser. All agree
...
for ts in "${TSMS[@]}"; do
  compare4 "$ts" "$(ts_run parsetime "$ts")" "$(rs_run parsetime "$ts")" \
    "$(go_run parsetime "$ts")" "$(py_run parsetime "$ts")"
done
check_fail "Cross-language timestamp MISMATCH."
echo "All timestamp edges: TS, Rust, Go and Python agree."

echo
echo "Kernel decisions (K1 + K2 vectors): TS vs Rust vs Go vs Python"
...
for vec in "$ROOT"/vectors/kernel/k1-*.json "$ROOT"/vectors/kernel/k2-*.json; do
  name="$(basename "$vec")"
  compare4 "$name" "$(ts_run decide "$vec" "$ROOT")" "$(rs_run decide "$vec")" \
    "$(go_run decide "$vec")" "$(py_run decide "$vec")"
done
check_fail "Cross-language MISMATCH."
echo "All K1+K2 vectors: kernel decisions are identical across stacks."

echo
echo "Settler execution receipts (S1 vectors): TS vs Rust vs Go vs Python"
...
for vec in "$ROOT"/vectors/settlers/s1-*.json; do
  name="$(basename "$vec")"
  compare4 "$name" "$(ts_run xr "$vec")" "$(rs_run xr "$vec")" \
    "$(go_run xr "$vec")" "$(py_run xr "$vec")"
done
check_fail "Cross-language MISMATCH."
echo "All S1 vectors: execution receipts are identical across stacks."

echo
echo "Cross-language agreement holds end-to-end across four independent stacks:"
